...
export default function LaunchPage() {
  ...
  const handleInputChange = (field: keyof FormData, value: any) => {
    setForm((prev) => {
      if (field === 'phaseThresholds') {
        return { ...prev, phaseThresholds: { ...prev.phaseThresholds, ...value } };
      }
  ...
  const handleDeploy = async () => {
    ...
    try {
      const factory = getSignedContract(CONTRACT_ADDRESSES.LAUNCH_FACTORY, LAUNCH_FACTORY_ABI, wallet.signer);
      const params = [
        form.name, form.symbol,
        parseUnits(form.totalSupply || '1000000', 18),
        parseUnits(form.initialLiquidity || '0', 18),
        BigInt(Math.round(form.initialSellTax * 100)),
      ...
      const tx = await factory.createLaunch(params, { value: bnbValue, gasLimit: 15_000_000 });
      setTxHash(tx.hash);
      ...
      if (receipt) {
        ...
        if (tokenAddress) {
          ...
          try {
            ...
            const [ts, st, bt] = await Promise.all([tokenContract.totalSupply(), tokenContract.sellTax(), tokenContract.buyTax()]);
            totalSupply = formatUnits(ts, 18); sellTax = Number(st) / 100; buyTax = Number(bt) / 100;
          ...
          await postLaunch({
            tokenAddress, vaultAddress, controllerAddress, governanceAddress, ammPairAddress,
            name: form.name, symbol: form.symbol, totalSupply, sellTax, buyTax,
            phase, phaseName: PHASE_NAMES[phase] || 'Genesis', mss, deployer: wallet.address, blockNumber: receipt.blockNumber || 0,
          });
          console.log('[Launch] Token registered in backend!');
        }
        router.push('/ai-agents/explore');
      }
    } catch (err: any) {
        ...
        {!isConnected && (
          <div className="luxury-card border-gold/20 p-12 mb-16 flex items-center gap-8 bg-secondary/80 backdrop-blur-xl">
            ...
            <div className="flex-1">
              ...
              <button onClick={connectWallet} className="btn-primary">Authorize Wallet</button>
            </div>
          </div>
        )}
        ...
        <div className="space-y-12 opacity-0 animate-in fade-in slide-in-from-bottom-5 duration-1000 fill-mode-forwards" style={{ animationDelay: '200ms' }}>
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-1">
              ...
              <p className="text-muted/60 text-sm leading-relaxed">Define the primary cryptographic identifiers and issuance parameters for your strategic asset.</p>
            </div>
              ...
              <div className="luxury-card p-10 bg-secondary/30">
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <input type="text" value={form.name} onChange={(e) => handleInputChange('name', e.target.value)} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" placeholder="e.g. EvoLaunch Prime" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Ticker Symbol</label>
                    <input type="text" value={form.symbol} onChange={(e) => handleInputChange('symbol', e.target.value)} maxLength={10} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium uppercase font-mono" placeholder="EVO" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Total Issuance</label>
                    <input type="number" value={form.totalSupply} onChange={(e) => handleInputChange('totalSupply', e.target.value)} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Initial Liquidity (BNB)</label>
                    <input type="number" step="0.01" min="0" value={form.initialLiquidityBnb} onChange={(e) => handleInputChange('initialLiquidityBnb', e.target.value)} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" placeholder="e.g. 0.05" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Initial Liquidity (Tokens)</label>
                    <input type="number" value={form.initialLiquidity} onChange={(e) => handleInputChange('initialLiquidity', e.target.value)} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-1">
              <h2 className="text-2xl font-bold text-primary mb-4 tracking-tight flex items-center gap-3"><Zap size={22} className="text-gold" /> Tax Engine</h2>
              <p className="text-muted/60 text-sm leading-relaxed">Configure the dynamic tax bounds. The Neural Core will optimize within these institutional limits.</p>
            </div>
              ...
              <div className="luxury-card p-10 bg-secondary/30">
                <div className="grid md:grid-cols-2 gap-x-10 gap-y-8">
                  <div className="space-y-3">
                    ...
                    <input type="number" value={form.initialSellTax} onChange={(e) => handleInputChange('initialSellTax', parseFloat(e.target.value))} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Initial Buy Tax (%)</label>
                    <input type="number" value={form.initialBuyTax} onChange={(e) => handleInputChange('initialBuyTax', parseFloat(e.target.value))} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Minimum Tax Floor</label>
                    <input type="number" value={form.minTax} onChange={(e) => handleInputChange('minTax', parseFloat(e.target.value))} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Maximum Tax Ceiling</label>
                    <input type="number" value={form.maxTax} onChange={(e) => handleInputChange('maxTax', parseFloat(e.target.value))} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-1">
              <h2 className="text-2xl font-bold text-primary mb-4 tracking-tight flex items-center gap-3"><Lock size={22} className="text-gold" /> Guardrails</h2>
              <p className="text-muted/60 text-sm leading-relaxed">Mitigate whale concentration Risk through per-mandate transaction and wallet capacity limits.</p>
            </div>
            <div className="lg:col-span-2">
              <div className="luxury-card p-10 bg-secondary/30">
                <div className="grid md:grid-cols-2 gap-x-10 gap-y-8">
                  <div className="space-y-3">
                    ...
                    <input type="number" value={form.initialMaxTx} onChange={(e) => handleInputChange('initialMaxTx', e.target.value)} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gold uppercase tracking-[0.1em] ml-2">Max Wallet Capacity</label>
                    <input type="number" value={form.initialMaxWallet} onChange={(e) => handleInputChange('initialMaxWallet', e.target.value)} className="w-full bg-background border border-gold/[0.08] rounded-xl px-6 py-4 focus:border-gold/30 outline-none transition-all text-primary font-medium" />
                  </div>
                </div>
              </div>
            </div>
          ...
          {validationWarnings.length > 0 && (
            <div className="luxury-card border-status-warning/20 p-8 bg-status-warning/5">
              <h3 className="font-bold text-status-warning mb-4 flex items-center gap-3 tracking-tight">
                <AlertCircle size={20} /> Strategic Validation Warnings
              </h3>
              <ul className="grid md:grid-cols-2 gap-4">
                {validationWarnings.map((warning, idx) => (
                  <li key={idx} className="text-status-warning/80 text-xs font-bold uppercase tracking-widest flex items-center gap-3">
                    <span className="w-1.5 h-1.5 rounded-full bg-status-warning" /> {warning}
                  </li>
                ))}
              </ul>
            </div>
            ...
            {txHash && (
              <div className="luxury-card border-gold/20 p-6 bg-gold/5 mb-8 flex flex-col gap-3">
                <div className="flex items-center gap-4 text-gold">
                  <CheckCircle size={20} />
                  <span className="text-sm font-bold uppercase tracking-widest">Transaction Propagating</span>
                </div>
                <code className="text-xs text-gold/60 font-mono block truncate py-2 px-4 bg-background/50 rounded-lg">{txHash}</code>
              </div>
            )}
            <button onClick={handleDeploy} disabled={loading || !isConnected || validationWarnings.length > 0} className="btn-primary w-full py-6 text-base tracking-[0.2em]">
              {loading ? (
